...
vector<string> IHM::choixDepartArrivee(Metro metro)
{
    ...
    for(unsigned int i = 0; i < liste_lignes.size(); ++i){
    ...
    for(int m = 0; m < 2; m++)
    {
        ...
        colorerEcran(14); //jaune
        cout << id_ligne[0];
        for(unsigned int i = 1; i < (id_ligne.size()-1); ++i){
            cout << ", " << id_ligne[i];
        }
        ...
        for(unsigned int i=0; i < lignes_entree.size(); ++i)
        {
            for(unsigned int j=0; j < lignes_entree[i]->getArretsLigne().size(); ++j)
            {
        ...
        for (unsigned int i = 0; i < arrets_entree_libelles.size(); ++i)
        {
            colorerEcran(14); //jaune
            cout<< i << " : ";
            colorerEcran(15); //blanc
        ...
        while((!((long unsigned int)menu_arret_depart >=0) & ((long unsigned int)menu_arret_depart <  arrets_entree_libelles.size()))){
            cout << "Choix incorrect, veuillez essayer de nouveau :" << endl;
            cin>>menu_arret_depart;
        }
...
void IHM::afficherItineraire(Itineraire itineraire)
{
    ...
    for(unsigned int i=1; i < (it_simplifie.size()-1); ++i){
        colorerEcran(11); //orange
